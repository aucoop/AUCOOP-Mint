(function () {
  ...
  document.querySelectorAll("a[hreflang]").forEach(function (link) {
    link.addEventListener("click", function () {
      ...
      try {
        ...
        // The link still works when the browser blocks local storage.
      }
    });
  });

  // Automatic detection belongs only on the English home page. Localized
  // pages and links to specific English pages must always stay where they are.
  ...
  if (normalizedPath(currentUrl.pathname) !== normalizedPath(englishHomeUrl.pathname)) {
    return;
  }
  ...
  try {
    ...
    // Browser language detection below does not need local storage.
  }
  ...
  if (!preferredLanguage) {
    ...
    preferredLanguage =
      browserLanguages.map(languageCode).find(function (language) {
